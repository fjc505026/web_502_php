...
foreach ($unitID_array as $key => $value) {      // construct Time table data
  $query="SELECT   act.activity_id, act.day,act.start_time,act.end_time, u.unit_code FROM activity act JOIN units u ON  act.unit_id=u.id WHERE act.type='tutorial'AND act.unit_id=".$value.";";
  $result = $mysqli->query($query);
  $Tut_allocated=false;
  while ($row = $result->fetch_array(MYSQLI_ASSOC)){
    ...
    if(in_array($row['activity_id'], $act_array,true)){
        ...
        $myArr[] = array('Day'=>$row['day'], 'Start'=>$row['start_time'],"End"=>$row['end_time'],"Code"=>$row['unit_code'],"Name"=>"Tutorial", "Coordinator"=>"none");
        $Tut_allocated=true;
      }
  }
